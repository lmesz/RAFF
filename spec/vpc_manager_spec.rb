...
describe 'VpcManager create_vpc_if_not_exists' do
  context 'when called and not found' do
    it 'creates one and return its id' do
      ...
      expect(vpc_mock).to receive(:modify_attribute).with(enable_dns_hostnames: { value: true })
  ...
  context 'when error happens' do
    it 'throws VpcManagerException' do
      ...
      ec2_mock = double('ec2')
      allow(ec2_mock).to receive(:create_vpc).and_raise(Aws::EC2::Errors::ServiceError.new nil, "Just an error")
