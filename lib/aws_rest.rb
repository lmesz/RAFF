...
class AwsRest < Sinatra::Base
    ...
    post '/deploy/:instance_name' do
      if not @aws_drupal_cluster_handler.deploy(params[:instance_name])
        content_type :json
        status 404
    ...
    post '/stop/:instance_name' do
      if not @aws_drupal_cluster_handler.stop(params[:instance_name])
        content_type :json
        status 404
        {:result => 'error', :message => 'Instance not found'}.to_json
      else
    ...
    post '/terminate/:instance_name' do
      if not @aws_drupal_cluster_handler.terminate(params[:instance_name])
        content_type :json
        status 404
        {:result => 'error', :message => 'Instance not found'}.to_json
      else
